...
// 2. 마커 꼬리 (삼각형)
        ...
        path.moveTo(width / 4f, width * 0.8f);

        path.lineTo(width / 2f, height);

        path.lineTo(width * 0.75f, width * 0.8f);
...
// 4. 숫자
        ...
        float y = (width / 2f) - bounds.centerY();
...
// ★ 수정: AddressCallback 사용으로 타입 오류 해결
    ...
    private void showMapTapDialog(LatLng latLng, String address) {
        ...
        etTime.setOnClickListener(v -> {
            ...
            int hour = c.get(Calendar.YEAR) == 0 ? 9 : c.get(Calendar.HOUR_OF_DAY);
